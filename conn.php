...
<?php
  ...
  // mysql接続
   ...
   // 食品一覧を取得
   $stmt3 = $db->query("SELECT DATE_FORMAT(date_time, '%Y-%m-%d') AS time, MAX(total) AS sum FROM chart WHERE date_time >= (NOW() - INTERVAL 7 DAY) GROUP BY DATE_FORMAT(date_time, '%Y%m%d')");
